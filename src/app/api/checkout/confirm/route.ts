...
export async function POST(request: NextRequest) {
  ...
  if (session.metadata?.orderId !== orderId || session.metadata?.buyerId !== auth.user.id) {
  ...
  const result = await markOrderPaymentHeldFromStripe(auth.adminClient, orderId, session.id);
  if (!result.ok) {
